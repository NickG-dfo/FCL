...
class SR{
	...
	T CustomRecruitment(T, const NumericVector);
	...
	public:
		...
		SR(const String &Rec_option, const T &R_std, 
		   ...
		   const int y, T Max_R, bool bc, bool RKseed): 
			...
			SPR = RP<T>(LHC, this->recruitment).SPR(0.); // Might work
			...
			else if( str_is(Rec_option, "Custom") ){ this->recruitment = SR::CustomRecruitment<T>; }
