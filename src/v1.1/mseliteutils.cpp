...
struct AR1d{
		...
		NumericVector operator() (NumericVector &X, T &phi, T &sig = 0.){
			...
			T sig0 = std / std::sqrt( 1.-std::pow(phi, 2.) );
			for(int i = 0; i < nforward; i++){
				if(i < n){
					Y(i) = X(i) + R::rnorm(0., sig0);
				}else{
					Y(i) += phi * Y(i-n) + R::rnorm(0., sig);
				}
			}
			return Y;
		}
